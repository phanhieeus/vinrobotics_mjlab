...
from src.utils.export_deploy_cfg import export_deploy_cfg
...
class TrainConfig:
    ...
    debug: bool = False
    """Skip all logging and checkpoint saves (no W&B / TensorBoard run created)."""
...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
    ...
    runner_kwargs = {}
    runner = runner_cls(env, agent_cfg, None if cfg.debug else str(log_dir), device, **runner_kwargs)
    ...
    if rank == 0:
        ...
        shutil.copy(
            inspect.getfile(cfg.env.__class__),
            os.path.join(
                log_dir, "params", os.path.basename(inspect.getfile(cfg.env.__class__))
            ),
        )
